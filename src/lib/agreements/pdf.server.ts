...
export async function renderSignedAgreement(input: SignedAgreement): Promise<Uint8Array> {
  ...
  y -= 8;
  write("The signer checked every acknowledgment, including consent to electronic records and authorization to charge the payment method on file for the $299.00 annual fleet membership when this enrollment is a fleet, or for prepaid screens only when this enrollment is a hire screen.", { gap: 8 });
