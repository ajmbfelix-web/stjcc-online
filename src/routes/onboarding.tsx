...
function Onboarding() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined" && new URLSearchParams(window.location.search).get("complete") === "1") {
  ...
  if (returnedFromCheckout) {
    return <main className="grid min-h-dvh place-items-center bg-background px-4"><section className="w-full max-w-lg rounded-xl border border-border bg-card p-8"><Badge tone="ok">Payment returned</Badge><h1 className="mt-5 text-4xl">Thanks. We are confirming your setup.</h1><p className="mt-4 text-sm leading-relaxed text-muted-foreground">Stripe sent you back successfully. SJCC will mark the organization active after the signed billing webhook is processed.</p><Link to="/" className="mt-7 inline-flex items-center gap-2 text-sm text-accent hover:underline">Return to site <ArrowRight className="size-4" /></Link></section></main>;
  }
